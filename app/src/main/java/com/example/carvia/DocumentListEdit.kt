...
import com.example.carvia.adapter.OsagoEdit
...
class DocumentListEdit : AppCompatActivity(), OsagoEdit.ClickListener {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()){
                    for (osagoSnapshot in snapshot.children){
                        ...
                        if (osago!!.uid.equals(uid.toString())){
                            osagoArrayList.add(osago)
                        }
                    }
                    osagoRecyclerView.adapter = OsagoEdit(osagoArrayList, this@DocumentListEdit)
                }
            }
    ...
    override fun clickedDocs(osago: Osago) {
        val intent = Intent(this, SelectedDoc::class.java)
